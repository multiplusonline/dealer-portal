-- Dealers table (updated with profile picture)
...
-- Messages table (updated with better indexing)
...
CREATE TABLE chat_logs (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES dealers(id) ON DELETE CASCADE,
  action TEXT NOT NULL, -- 'message_sent', 'message_read', 'conversation_started'
  details JSONB,
  timestamp TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
('other-user-id', 'current-user-id', 'Ja, ik heb hem gisteren gedownload. Bedankt!', NOW() - INTERVAL '1 hour', true),
('current-user-id', 'other-user-id', 'Mooi! Heb je nog vragen over de nieuwe producten?', NOW() - INTERVAL '30 minutes', true),
('other-user-id', 'current-user-id', 'Ja, ik zou graag meer info willen over de technische specificaties van product X.', NOW() - INTERVAL '15 minutes', false),
('current-user-id', 'third-user-id', 'Hoi Piet, hoe gaat het met je project?', NOW() - INTERVAL '2 hours', true),
('third-user-id', 'current-user-id', 'Goed bezig! We zijn bijna klaar met de installatie.', NOW() - INTERVAL '1 hour', true);
